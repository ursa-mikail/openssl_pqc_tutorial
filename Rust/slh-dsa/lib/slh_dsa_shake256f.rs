...
fn main() {
    ...
    let method = String::from("SLH-DSA-SHAKE-256f");
    ...
    let (pk1, sk) = match slh_dsa_shake_256f::try_keygen() {
        Ok(keys) => keys,
        Err(e) => {
    ...
    let pk2 = match slh_dsa_shake_256f::PublicKey::try_from_bytes(&pk_send) {
        Ok(pk) => pk,
        Err(e) => {
