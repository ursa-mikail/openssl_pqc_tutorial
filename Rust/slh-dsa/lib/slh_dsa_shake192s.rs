...
fn main() {
    ...
    let method = String::from("SLH-DSA-SHAKE-192s");
    ...
    let (pk1, sk) = match slh_dsa_shake_192s::try_keygen() {
        Ok(keys) => keys,
        Err(e) => {
    ...
    let pk2 = match slh_dsa_shake_192s::PublicKey::try_from_bytes(&pk_send) {
        Ok(pk) => pk,
        Err(e) => {
