...
fn main() {
    ...
    // Use try_keygen() for version 0.4.1
    let (pk1, sk) = match slh_dsa_shake_128f::try_keygen() {
        Ok(keys) => keys,
        Err(e) => {
    ...
    // Use try_sign with context and hedge parameters
    ...
    let pk2 = match slh_dsa_shake_128f::PublicKey::try_from_bytes(&pk_send) {
        Ok(pk) => pk,
        Err(e) => {
    ...
    // Use verify() for version 0.4.1 - returns bool directly
