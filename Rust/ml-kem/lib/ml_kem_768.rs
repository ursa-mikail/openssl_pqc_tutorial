...
fn main() {
    let method = String::from("ML-KEM-768");

    println!("\nMethod: {}\n", method);

    // Alice runs `try_keygen()` and then serializes the encaps key `ek` for Bob (to bytes).
    let (alice_ek, alice_dk) = match ml_kem_768::KG::try_keygen() {
        Ok(keys) => keys,
        Err(e) => {
    ...
    let bob_ek = match ml_kem_768::EncapsKey::try_from_bytes(bob_ek_bytes) {
        Ok(ek) => ek,
        Err(e) => {
    ...
    let alice_ct = match ml_kem_768::CipherText::try_from_bytes(alice_ct_bytes) {
        Ok(ct) => ct,
        Err(e) => {
